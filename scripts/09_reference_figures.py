...
def grouped_bars(out_dir: Path, name: str, panel: dict, groups: Sequence[str],
                 ...
                 ncol: int = 5, legend_handletextpad: float = 0.5,
                 legend_columnspacing: float = 1.0) -> None:
    paper_style()
    ...
    leg = ax.legend(loc="upper center", ncol=ncol, frameon=False, borderaxespad=0.0,
                    columnspacing=legend_columnspacing, handlelength=2.0,
                    handletextpad=legend_handletextpad, fontsize=17.0)
...
def figure_entropy_sweep(out_dir: Path, dirs: Sequence[Path]) -> None:
    ...
    # Fig8's legend is the one bar legend of the reference set that is laid out with a wider
    # handle-text gap and a wider inter-column gap than fig2/3/4 (which use 0.5 em / 1.0 em).
    # Target geometry measured on D:\figures: patch -> text 23 px, text -> next patch 51.5 px.
    # With the saved font size (17 pt at 100 dpi = 23.62 px per em) and the glyph side
    # bearings of this font (+3.0 px after the patch, +1.5 px after the text), that is
    # handletextpad 0.83 em and columnspacing 2.12 em.
    grouped_bars(out_dir, "fig08_entropy_sharpe_ratio.png", panel, xlabels, values,
                 ...
                 legend_handletextpad=0.83, legend_columnspacing=2.12)
...
* 柱状图：柱簇总宽 = min(0.78, 0.18×系列数) 个横轴单位、柱子紧邻（间隙 0），
  ...
  图例 `loc='upper center'` 且贴住轴顶（`borderaxespad=0`），且**不参与 `tight_layout`**
  （`set_in_layout(False)`）—— 否则居中的 5 列宽图例会把绘图区压窄 ≈160 px
  （对照图实测绘图区宽 1192 px，我们设了 `set_in_layout(False)` 后为 1193 px）。
  图例内部间距按张量取：Fig2/3/4 用 `columnspacing=1.0` + `handletextpad=0.5`；
  **Fig8 的图例间距明显更宽**（对照图实测「色块→文字」23 px、「文字→下一个色块」51 px），
  按 17 pt 字号（1 em = 23.62 px）与字形侧边距反解得 `handletextpad=0.83`、
  `columnspacing=2.12`，复测为 22 px / 52 px（±1 px，属 JPEG 对照图的量测误差量级）。
...
8. **Fig8 的柱子**：对照图里所有夏普都为正、柱子贴住下边界；我们的夏普在三折上跨零
   ...
   见 §9.12.1），所以下边界落在最小值上、柱高有正有负。**这是 Fig8 与对照图看起来差得
   最大的地方，但它是数据差、不是画法差**：15 根柱子的矩形下沿在数据坐标上全部等于 0，
   即全部从同一根 y=0 基线出发（程序化逐柱核对），只是这根 0 基线此时落在坐标区中部
   （≈277 px）而非贴住下边界。其余几何量（柱簇宽度、x 轴范围、顶部 7% 留白、
   图例 17 pt / 色块 2.0 字号 / 图例内部间距）已按对照图逐像素对齐。
