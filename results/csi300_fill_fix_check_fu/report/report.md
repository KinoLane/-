# 端到端可微 Mean-CVaR 投资组合学习 — 实验结果报告

- 运行目录：`csi300_fill_fix_check_fu`
- 运行耗时：57.6 分钟，作业 1/1 成功
- 样本：沪深 300 动态成分股，测试窗口 1 折拼接，VaR/CVaR 置信水平 95%
- 成本假设：单边 15.00 bp，线性；换手率按单边（½·Σ|Δw|）统计，并按实际调仓频率年化

## 0. 折划分

| 折 | 训练起 | 训练止 | 验证起 | 验证止 | 测试起 | 测试止 |
|---|---|---|---|---|---|---|
| f2_2020_2021 | 2010-01-01 | 2018-12-31 | 2019-01-01 | 2019-12-31 | 2020-01-01 | 2021-12-31 |

## 1. 主结果（所有折的样本外日收益拼接后重算）

| 实验 | 类别 | 年化净收益 | 年化波动 | Sharpe | 最大回撤 | CVaR95(日) | 年化换手 | 有效持仓 | 成本拖累 |
|---|---|---|---|---|---|---|---|---|---|
| full | e2e | 3.92% | 14.61% | 0.27 | -11.94% | 2.17% | 5.48 | 18.83 | 1.72% |

### 1.2 持仓数量与集中度（Plan §6 要求项）

逐次调仓统计后取均值；HHI = Σw²，Top-5 与最大权重为占组合比例。

| 实验 | 类别 | 持仓数(>1e-3) | 有效持仓 1/Σw² | HHI Σw² | 前5大权重 | 最大单一权重 |
|---|---|---|---|---|---|---|
| full | e2e | 29.08 | 18.83 | 0.05 | 37.19% | 8.52% |

## 2. 分折结果

### 2.1 Sharpe

| 实验 | 折 | Sharpe | 年化净收益 | 最大回撤 | CVaR95(日) |
|---|---|---|---|---|---|
| full | f2_2020_2021 | 0.27 | 3.92% | -11.94% | 2.17% |

## 3. 选择层诊断

| 实验 | 折 | 选择器 | 编码器哈希 | 期数 | 可投资 | logit 极差 | 稀疏临界 1/n | 极差<1/n 占比 | π 支撑集 | π 有效数 | 账本持仓 | 账本有效持仓 |
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| full | f2_2020_2021 | sparsemax | e0248f3ba0fafd1e | 24 | 293.83 | 4.46e-02 | 3.40e-03 | 0.00% | 35.42 | 23.39 | 29.08 | 18.83 |

> 诊断口径：重放测试期并只跑编码器（`run_layer=False`），以空账本（`prev = 0`）隔离选择层本身；`book_*` 列读自回测写出的 `weights.csv`，因此与 `pi_*` 的差别来自不可卖出持仓的结转与优化层。

## 4. 结论

1. **整体排序**：在全部 1 折测试窗口拼接后的样本上，夏普比率最高的是 `full`（Sharpe 0.27，年化净收益 3.92%，最大回撤 -11.94%）。基准（等权 1/N）的年化净收益为 n/a，传统历史情景 Mean-CVaR 为 n/a。
6. **风险与成本**：`full` 的日 CVaR(95%) 为 2.17%，年化波动 14.61%，平均持仓 29.1 只、有效持仓 18.8 只，年化单边换手 5.48，成本拖累（毛收益−净收益）1.72%。
7. **跨折稳定性**：只有 `full` 在全部 1 折上夏普为正；正夏普折数最多的是 `full`（1/1 折，最差 +0.27 / 最好 +0.27）。所有策略在不同年份区间上都有明显波动，单折结论不足以支撑泛化性判断。
8. **解读与局限**：结论受限于（a）单一指数（沪深 300 动态成分股）与单一成本假设（单边 15 bp 线性成本，未建模冲击成本与涨跌停延迟成交）；（b）测试窗口只有 3 折、共约 8 年，统计功效有限；（c）场景生成是高斯参数化模型，CVaR 只对模型隐含分布稳健；（d）深度网络的随机性使同一配置重复训练仍会有数个百分点差异，报告中的差异若小于该量级不应视为真实效应；（e）资产槽位数 `n_max` 由每折的可投资域决定（f1/f2/f3 = 154/155/165），它通过 `Dropout` 的随机数消耗量影响训练轨迹，因此**跨折的绝对值不可直接横向比较**（仓库内同一折的所有实验共用同一个 `n_max`，折内比较仍然成立）；（f）选择层的稀疏不等于组合稀疏——单票 10% 上限会把权重摊到更多票上，有效持仓应看回测输出的 `eff_holdings`（`full` 为 18.8 只），而不是 π 的支撑集大小。

## 5. 图表

![net equity](figures/fig_equity.png)

![drawdown](figures/fig_drawdown.png)

![metric bars](figures/fig_metrics.png)

![risk/return](figures/fig_risk_return.png)

![sharpe heatmap](figures/fig_sharpe_heatmap.png)

![training history](figures/fig_history_full.png)

![cost drag](figures/fig_cost_full.png)

![portfolio profile](figures/fig_profile_full.png)

## 6. 复现

```bash
python scripts/01_prepare_data.py --validate
python scripts/02_run_experiments.py --config results/csi300_fill_fix_check_fu/config.yaml --workers 4
python scripts/04_selection_diagnostics.py --run results/csi300_fill_fix_check_fu
python scripts/03_report.py --run results/csi300_fill_fix_check_fu
```

> 每次运行都会把**解析后的完整配置**复制到运行目录的 `config.yaml`，因此上面第二条命令对基线网格（`configs/csi300.yaml`）、固定分数规则网格（`configs/csi300_score_rule.yaml`）和全样本扩展网格（`configs/csi300_full_universe.yaml`）都成立。
